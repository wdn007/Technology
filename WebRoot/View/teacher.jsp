...
<head>
    ...
    <script type="text/javascript">
        ...
        function newStuInfo() {
            ...
            $('input[name=jobID]').attr('readonly', false);
            $('#fm').form('clear');
            url = 'Teacher/addTecInfo';
        }
        function editStuInfo() {
            var row = $('#dg').datagrid('getSelected');
            if (row) {
                $('#dlg').dialog('open').dialog('setTitle', '编辑用户');
                $('input[name=jobID]').attr('readonly', true);
                $('#fm').form('load', row);
                url = 'Teacher/updateTecInfo';
            }
        }
        ...
        function removeStuInfo() {
            ...
            if (row) {
                $.messager.confirm('Confirm', '你确定要删除用户吗?', function (r) {
                    if (r) {
                        $.post('Teacher/removeTecInfo', {jobID: row.jobID}, function (result) {
                            if (result.success) {
                                $.messager.show({
                                    title: 'Info',
                                    msg: result.msg,
...
<table id="dg" title="My Users" class="easyui-datagrid" style="width:1000px;height:450px"
       url="Teacher/listTecInfo"
       toolbar="#toolbar" pagination="true"
       rownumbers="true" fitColumns="true" singleSelect="true">
    <thead>
    <tr>
        <th field="jobID" width="50">工号</th>
        <th field="username" width="50">姓名</th>
        <th field="password" width="50">密码</th>
        <th field="sex" width="70">性别</th>
        <th field="telNo" width="70">手机号码</th>
        <th field="email" width="50">邮箱</th>
        <th field="academy" width="50">学院</th>
        <th field="duties" width="70">职称</th>
...
<div id="toolbar">
    ...
    <div>
        <span>工号:</span>
        <input id="jobID" style="line-height:26px;border:1px solid #ccc">
        <span>姓名:</span>
...
<div id="dlg" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px"
    ...
    <div class="ftitle">Teacher's Information</div>
    <form id="fm" method="post" novalidate>
        <div class="fitem">
            <label>工号:</label>
            <input name="jobID" class="easyui-validatebox" required="true">
        </div>
        <div class="fitem">
            <label>姓名:</label>
        ...
        <div class="fitem">
            <label>职称:</label>
            <input name="duties" class="easyui-validatebox" required="true">
        </div>
        <div class="fitem">
            <label>备注:</label>
